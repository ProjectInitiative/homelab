#!/usr/bin/env bash
# boot-shape-warmup.sh — burn DSpark / sampler shapes after /health.
#
# DeepSeek-V4.1-Flash DSpark k=5:
#   BLOCK_SIZE = min(256, next_pow2(scheduled_tokens + num_query_per_req))
#   num_query_per_req = 1 + k = 6
# BLOCK 8 is reachable (s=1 → 7 → 8). Include it.
#
...
#   GLM53_WARMUP_DSPARK_K          speculative tokens (default 5)
...
DSPARK_K="${GLM53_WARMUP_DSPARK_K:-5}"
...
# k=5 → +6. Hit BLOCK in {8,16,32,64,128,256}.
LADDER_S=(1 10 26 58 122 250)
...
fire() {
  ...
  if [ "$profile" = "serve-default" ]; then
    payload='{"model":"'"$MODEL"'","messages":[{"role":"user","content":"'"$prompt"'"}],"temperature":0,"chat_template_kwargs":{"enable_thinking":false}}'
...
echo "boot-shape-warmup: sweeping DSpark k=${DSPARK_K} / sampler shapes"
total_t0=$(date +%s)
