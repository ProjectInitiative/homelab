...
This file is copied into the image at /opt/dsv41/test_exl3_overlay.py and is
...
def _check_quant_registry() -> None:
    ...
    assert abs(float(cfg.avg_bits) - 2.9) < 1e-6
    cfg_avg = cfg_cls.from_config(
        {"quant_method": "exl3", "avg_bits": 2.9, "bits": 3, "codebook": "mul1"}
    )
    assert abs(float(cfg_avg.avg_bits) - 2.9) < 1e-6
    assert cfg.codebook == "mul1"
    assert cfg.override_quantization_method({"quant_method": "exl3"}, None) == "exl3"
    huge = {"quant_method": "exl3", "bits": 2.9, "codebook": "mul1", "tensor_storage": {"x": 1}}
    cfg2 = cfg_cls.from_config(huge)
    assert "tensor_storage" not in cfg2.raw_config

    import inspect
    from vllm.model_executor.layers.linear import LinearMethodBase
    from vllm.model_executor.layers.quantization.exl3 import Exl3LinearMethod

    base = inspect.signature(LinearMethodBase.create_weights)
    ours = inspect.signature(Exl3LinearMethod.create_weights)
    for name in (
        ...
        assert name in ours.parameters, (name, tuple(ours.parameters))
        assert name in base.parameters, name
    print("exl3 registry OK", flush=True)


def _check_k_map() -> None:
    from vllm.model_executor.layers.quantization.exl3 import k_for_prefix

    assert k_for_prefix("model.layers.0.mlp.experts") == 3
    assert k_for_prefix("model.layers.18.mlp.experts") == 2
    assert k_for_prefix("model.layers.1.engram.wkv") == 5
    assert k_for_prefix("model.layers.14.engram.wkv") == 4
    assert k_for_prefix("mtp.0.ffn.experts") == 4
    assert k_for_prefix("lm_head") == 6
    assert k_for_prefix("language_model.lm_head") == 6
    assert k_for_prefix("model.layers.0.self_attn.o_proj") != 6
    assert k_for_prefix("layers.14.attn.indexer.wk") == 8
    assert k_for_prefix("layers.14.attn.indexer.wq_b") == 5
    print("exl3 k_map OK", flush=True)
...
def _check_lm_head_packed_shapes() -> None:
    """Checkpoint head.trellis is [320, 8080, 96] = K=5120, N=129280, bits=6."""
    import torch
    from vllm.model_executor.layers.quantization.exl3 import (
        Exl3Config,
        Exl3LinearMethod,
    ...
    method = Exl3LinearMethod(cfg, prefix="language_model.lm_head")
    assert method.k == 6, method.k
    ...
    assert tuple(layer.trellis.shape) == (320, 4040, 96), tuple(layer.trellis.shape)
    assert tuple(layer.suh.shape) == (5120,)
    assert tuple(layer.svh.shape) == (64640,)
    assert layer._exl3_shard == "col"
    marker = torch.tensor(-2082680531, dtype=torch.int32)  # CPU
    method._load(layer.mul1, marker, "language_model.lm_head.mul1")
    assert int(layer.mul1.item()) == -2082680531
    fused = torch.nn.Module()
    fused_m = Exl3LinearMethod(cfg, prefix="language_model.model.layers.0.attn.fused_wqa_wkv")
    fused_m.create_weights(fused, 5120, [1280, 512], 5120, 1792, torch.bfloat16)
    fused_m._load(fused.svh, torch.zeros(1280, dtype=torch.float16), 0)
    fused_m._load(fused.svh, torch.zeros(512, dtype=torch.float16), 1)
    fused_m._load(fused.suh, torch.zeros(5120, dtype=torch.float16), 0)
    fused_m._load(fused.suh, torch.zeros(5120, dtype=torch.float16), 1)
    fused_m._load(fused.trellis, torch.zeros(320, 80, 96, dtype=torch.int16), 0)
    fused_m._load(fused.trellis, torch.zeros(320, 32, 96, dtype=torch.int16), 1)
    fused_m._load(fused.mul1, marker, 0)
    fused_m._load(fused.mul1, marker, 1)
    assert fused._exl3_fused_stash[0]["svh"].shape == (1280,)
    assert fused._exl3_fused_stash[1]["svh"].shape == (512,)
    assert fused._exl3_fused_stash[0]["trellis"].shape[-1] == 96
    # The stash is the source of truth for fused linears; the merged packed
    # Parameters from create_weights must not keep their (UMA) storage.
    for pname in ("trellis", "suh", "svh"):
        assert getattr(fused, pname).numel() == 0, (pname, getattr(fused, pname).shape)
    # Non-fused path still fills the parameter in place (no stash).
    assert not getattr(layer, "_exl3_fused_stash", None)
    print("exl3 lm_head packed shapes OK", flush=True)


def _check_ext_arch() -> None:
    import torch  # noqa: F401  — libc10 must load before the compiled ext
...
def _check_e2_diag_static() -> None:
    ...
    try:
        ...
        # v2 grouped kernels gather gate and up separately, so a checkpoint
        # without a shared gate/up sign vector still runs the grouped tier
        # (the E2/E1 tiers keep the shared-SUH cap).
        assert resolve_exl3_fat_tier(False, (True, True, True), True) == (
            "grouped",
            "grouped_ok",
        )
        tier, reason = resolve_exl3_fat_tier(
            False, (True, True, True), True, grouped_eligible=(False, "uncompiled_5mul1")
        )
        assert tier == "sorted" and reason.endswith("shared_suh_absent"), (tier, reason)
...
def _check_gpu_gemm() -> None:
    ...
    y = execute_exl3_linear(
        x, trellis, suh, svh, mcg, codebook="mcg", out_dtype=torch.float32
    )
...
def _overlay_codebook() -> str:
    from vllm.model_executor.layers.quantization.exl3 import Exl3Config

    return Exl3Config().codebook


def _tiny_layer_is_k4_mcg(layer) -> bool:
    """E2/E3 fat GEMMs TORCH_CHECK K==4 && mcg && !mul1. This overlay is mul1."""
    bits = int(getattr(layer, "_exl3_bits", 0) or 0)
    codebook = str(getattr(layer, "_exl3_codebook", "") or "")
    return bits == 4 and codebook == "mcg"
...
def _tiny_layer(device, n_exp: int = 3, hidden: int = 256, inter: int = 256):
    ...
    marker = (
        MUL1_MARKER_SIGNED_INT32
        if method.codebook == "mul1"
        else MCG_MARKER_SIGNED_INT32
    )
    packed13 = getattr(layer, "w13_mul1", layer.w13_mcg)
    packed2 = getattr(layer, "w2_mul1", layer.w2_mcg)
    ...
    with torch.no_grad():
        ...
        packed13.fill_(marker)
        packed2.fill_(marker)
...
def _check_fused_fat_and_row_tile(device) -> None:
    ...
    try:
        ...
        err_kernel = None
        if hasattr(exllamav3_ext, "exl3_fat_gemm") and _tiny_layer_is_k4_mcg(layer):
            os.environ["EXL3_FAT_BATCHED"] = "0"
            os.environ["EXL3_FAT_KERNEL"] = "1"
            ...
            os.environ["EXL3_FAT_KERNEL"] = "0"
        elif hasattr(exllamav3_ext, "exl3_fat_gemm"):
            print(
                "exl3 fat kernel MoE apply skipped "
                f"(tiny layer codebook={getattr(layer, '_exl3_codebook', '?')} "
                f"K={getattr(layer, '_exl3_bits', '?')}; fat GEMM is K4/MCG)",
                flush=True,
            )
...
def _check_mixed_thin_fat(device) -> None:
    ...
    try:
        ...
        if hasattr(exllamav3_ext, "exl3_fat_gemm") and _tiny_layer_is_k4_mcg(layer):
            ...
            print(
                "exl3 mixed thin+fat kernel skipped "
                f"(tiny layer codebook={getattr(layer, '_exl3_codebook', '?')})",
                flush=True,
            )
    finally:
...
def _check_e2_diag(device) -> None:
    ...
    if not hasattr(exllamav3_ext, "exl3_fat_gemm"):
        ...
        return
    if _overlay_codebook() != "mcg":
        print(
            "exl3 E2 diag kernel counters skipped "
            f"(overlay codebook={_overlay_codebook()}; fat GEMM is K4/MCG)",
            flush=True,
        )
        return
...
def _check_grouped_fat(device) -> None:
    ...
    if _overlay_codebook() != "mcg":
        print(
            "exl3 E3 grouped MoE apply skipped "
            f"(overlay codebook={_overlay_codebook()}; grouped kernels are K4/MCG)",
            flush=True,
        )
        return
...
def main() -> int:
    ...
    print("dsv41 EXL3 overlay verify OK", flush=True)
    return 0
